...
class Firebase {
    ...
    constructor(_db, _auth, domain = "firebaseio.com") {
        ...
        if (_auth != null && strip(_auth) == "") _auth=null;
        auth = _auth;
        data = {}; 
        callbacks = {};
    }
...
const FIREBASENAME = "examplefirebase";
const FIREBASESECRET = "";

firebase <- Firebase(FIREBASENAME, FIREBASESECRET);

firebase.on("/bpm", function(path, state) {
    if (state != null) {
        device.send("bracelet", state);
    }
});

firebase.stream("/bracelet", true);
server.log("started streaming");

function newInterval(interval) {
    server.log("New interval: " + interval);
}

device.on("interval", newInterval)