...
const FIREBASE_NAME = "examplefirebase";
const FIREBASE_SECRET = "";

deviceId <- "";
firebase <- Firebase(FIREBASE_NAME, FIREBASE_SECRET);

function onDeviceId(_deviceId) {
    deviceId = _deviceId;
    server.log("Got Id: " + deviceId);
}

device.on("id", onDeviceId);

function buttonClicked(state) {
    server.log("device: " + deviceId + " got state: " + state);
    if (deviceId != "") {
        firebase.write("rolecall/" + deviceId, state) 
    }
}

device.on("button", buttonClicked);